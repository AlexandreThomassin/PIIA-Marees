...
print(os.getcwd())
data_path = "PIIA-Marees/Loire/Datas/"
...
ports = ['SN', 'Montoir', 'Paimboeuf', 'Cordemais', 'LP', 'NUB', 'NSAL']
other_features = ['Nantes Patm', 'SG Patm']
to_drop_X = []
to_drop_Y = []

for port in ports[1:]:
    for i in range(36, 0, -1):
        to_drop_X.append(f'{port} Hauteur (t-{i})')

for feature in other_features:
    for i in range(36, 0, -1):
        to_drop_X.append(f'{feature} (t-{i})')

for port in ports[1:]:
    to_drop_Y.append(f'{port} Hauteur (t)')
    for i in range(1, 6):
        to_drop_Y.append(f'{port} Hauteur (t+{i})')
...
X_tides = X_tides.drop(to_drop_X, axis = 1)
Y_tides = Y_tides.drop(to_drop_Y, axis = 1)
